...
from consultant.models import ConsultantToken, ConsultantPetitionToken
...
class ConsultantPetitionIsAuthenticated(BasePermission):
    """
    Allows access only to authenticated Consultants.
    """
    ...
    def has_permission(self, request, view):
        ...
        if len(auth) > 0:
            key = auth[1].decode()
            try:
                consultant_token = ConsultantPetitionToken.objects.select_related('consultant').get(key=key)
