...
def get_employee_info(queryset, user_type=None):
    if user_type == "supervisor":
        ...
        for interview in queryset:
            ...
            supervisor_info.update({"InterviewID": interview.id})
            supervisor_data.append(supervisor_info)
        return supervisor_data
    if user_type == "coder":
        coders_data = []
        for interview in queryset:
            coder_info = get_employee_info(interview.guests.filter(type__in=['Coder', 'Coder & Assistant']).values_list('user__employee_id', flat=True))
            if type(coder_info) is list:
                for info in coder_info:
                    info.update({"Round": f"Round-{interview.round}"})
                    info.update({"InterviewID": interview.id})
                    coders_data.append(info)
            else:
                coder_info.update({"Round": f"Round-{interview.round}"})
                coder_info.update({"InterviewID": interview.id})
                coders_data.append(coder_info)
        return coders_data
...
def get_stakeholders_involved(project):
    ...
    coders = project.submission.screening.exclude(status='cancelled')
    stakeholders = {
        "VP": get_employee_info(vp),
        "TeamLead": get_employee_info(team_lead),
        "Recruiter": get_employee_info(recruiter),
        "Coder": get_employee_info(coders, "coder"),
...
def share_po_stakeholder_info(project, request=None):
    try:
        month_id = int(f"{datetime.now().strftime('%Y')}{datetime.now().strftime('%m')}")
        conclusion_date_timestamp, conclusion_date = None, None
        ...
        end_date_timestamp, end_date_datetime = (get_timestamp(project.end_date), get_datetime(project.end_date)) \
            if project.end_date else (None, None)
        status_db_obj = project.statuses.filter(is_current=True).first()
        status_db_name = status_db_obj.status
        if status_db_name == 'complete' or "cancelled" in status_db_name or "terminated" in status_db_name:
            conclusion_date_timestamp, conclusion_date = (
                get_timestamp(status_db_obj.created), get_datetime(status_db_obj.created)
            )

        stakeholder_data = {
            "month_id": month_id,
            "ProjectID": project.id,
            "POEndDate": int(end_date_timestamp),
            "POEndDateStr": end_date_datetime,
            "JoinedDate": joined_at_timestamp,
            "JoinedDateStr": joined_at_datetime,
            "Client": project.submission.client,
            "POStartDate": start_date_timestamp,
            "ConclusionDateStr": conclusion_date,
            "POStartDateStr": start_date_datetime,
            "SubmissionID": project.submission.id,
            "OfferReceivedDate": received_at_timestamp,
            "ConclusionDate": conclusion_date_timestamp,
