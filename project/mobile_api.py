...
class TimeSheetViewSets(GenericViewSet, ListModelMixin, UpdateModelMixin, DestroyModelMixin):
    ...
    def history(self, request):
        ...
        try:
            project_status = ['joined', 'resigned', 'completed', 'client-fired', 'resigned-rate', 'terminated-other',
                              'resigned-location', 'resigned-full_time', 'client-fired-budget', 'resigned-technology',
                              'client-fired-security', 'client-fired-performance']

            project_ids = request.user.get_project().filter(status__in=project_status).values_list('id', flat=True)
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            project_status = ['joined', 'resigned', 'completed', 'client-fired', 'resigned-rate', 'terminated-other',
                              'resigned-location', 'resigned-full_time', 'client-fired-budget', 'resigned-technology',
                              'client-fired-security', 'client-fired-performance']
            projects = request.user.get_project().filter(status__in=project_status)
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            if project:
                ...
                timesheet = get_object_or_404(TimeSheet, project=project, id=kwargs.get('pk', None),
                                              status__in=['draft', 'rejected'], is_active=True)
