...
class ProjectViewSets(viewsets.ModelViewSet):
    ...
    def send_offer_received_mail(self, submission, scrum_master, marketer):
        try:
            ...
            to = [config.RELATIONS, config.FINANCE, config.RECRUITMENT, marketer.email,
                  marketer.team.email]
            cc = [recruiter] + config.SUPERADMIN
            if scrum_master:
    ...
    def support_mail(start_date, submission, scrum_master, marketer):
        try:
            ...
            cc = [config.RECRUITER, config.RELATIONS, recruiter, marketer.email, marketer.team.email]
    ...
    def po_mail(self, path, scrum_master_email, po_type):
        ...
        try:
            ...
            recruiter = self.submission.consultant.recruiter.email
            to = [config.RELATIONS, config.FINANCE, config.RECRUITMENT, config.LEGAL, marketer.team.email, marketer.email]
            cc = [recruiter] + config.SUPERADMIN
    ...
    def po_termination_or_cancellation_mail(self, scrum_master_email, po_type):
        ...
        try:
            ...
            to = [config.RELATIONS, config.FINANCE, config.RECRUITMENT, config.LEGAL, marketer.team.email, recruiter]
...
#         try:
#             project = get_object_or_404(Project, id=project_id)
#             prev_status = project.statuses.get(is_current=True)
...
# **Recruiter : **  {project.submission.consultant.recruiter.name}
# **CTB : **
# {supervisors}
