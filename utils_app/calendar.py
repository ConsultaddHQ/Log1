...
class GoogleCalendar:
    ...
    def book_calendar(self, data):
        try:
            ...
            event = service.events().insert(calendarId=os.environ.get('LOG1CALENDER_ID'), body=event, sendUpdates='all').execute()
            return event, "ok"
        except Exception as error:
    ...
    def update_calendar(self, event_id, data):
        try:
            service = self.calendar_con()
            event = self.get_body(data)
            updated_event = service.events().update(calendarId=os.environ.get('LOG1CALENDER_ID'), eventId=event_id, body=event,
