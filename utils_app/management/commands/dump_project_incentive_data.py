...
class Command(BaseCommand):
    def handle(self, *args, **options):
        try:
            count = 1
            url = "https://yd5inua2n5.execute-api.ap-south-1.amazonaws.com/incentive/trigger/project"
            # url = " http://localhost:3000/incentive/trigger/project"
            project_qs = Project.objects.filter(statuses__status='received', statuses__created__gt="2024-03-01")

            print(project_qs.count())
            for project in project_qs:
                incentive_json = share_po_stakeholder_info(project)
                dump_data = json.dumps(incentive_json)
                resp = requests.post(url, json=json.loads(dump_data))
