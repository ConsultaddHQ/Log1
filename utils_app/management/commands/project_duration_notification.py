...
class Command(BaseCommand):
    ...
    @staticmethod
    def send_project_completion_notification(obj, days):
        submission = obj.project.submission
        mail_data = {
            "to": [config.FINANCE], "bcc": [],
            'template': '../templates/project_completion.html',
            'subject': f"Congratulations!!! {submission.consultant.name}'s project with {submission.client} completed {days}",
        ...
        send_email(mail_data, "product@consultadd.com", None, True)
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for obj in associates_qs:
                self.send_project_completion_notification(obj, 160)
