...
from consultant.models import Consultant, ConsultantMarketing, ConsultantProfile
...
command - {command}\n
| Team Name | Scrum Master | Current Bench | Submission | Interview | Offer |
|:----------|:-------------|:--------------|:-----------|:----------|:------|
"""
        scrum_master = "Sudeep B."
        offer_count = Project.objects.filter(created__gte=start).exclude(statuses__status='new', statuses__is_current=True).count()
        ...
        text += f"| Consultadd | {scrum_master} | {bench_consultant} | {submission_count} | {interview_count} | {offer_count} |\n"
        ...
        for team in teams:
            ...
            interview_count = Interview.objects.filter(
                created__gte=start, submission__created_by__team__name__iexact=team.name
            ).exclude(status='cancelled').count()
            offer_count = Project.objects.filter(
                created__gte=start, submission__created_by__team__name__iexact=team.name
            ).exclude(statuses__status='new', statuses__is_current=True).count()
            ...
            text += f"| {team.name.title()} | {scrum_master} | {bench_consultant} | {submission_count} | {interview_count} | {offer_count} |\n"
        return text
...
"""
        ...
        scrum_master = "Sudeep B."
        bench_consultant = Consultant.objects.filter(status='on_bench').count()
        offer_count = Project.objects.filter(created__month=month, created__year=year).exclude(statuses__status='new', statuses__is_current=True).count()
        submission_count = Submission.objects.filter(
...
"""
            for user in employees:
                ...
                submission_count = Submission.objects.filter(created_by=user, created__gte=start).exclude(status='cancelled').count()
                interview_count = Interview.objects.filter(submission__created_by=user, created__gte=start).exclude(
