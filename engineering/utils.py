    ...
    try:
        ...
        for data in payload:
            ...
            while count < data['project']['bandwidth']:
                ...
                count += 1
        file.close()
        file_url = generate_s3_url(file.name, request, "Engineer Detail")
        return file_url
...
def get_team_structure_xlsx(payload, counts, request):
    try:
        ...
        writer.save()
        file_url = generate_s3_url(f'team_structure_{filename}.xlsx', request, "Engineer Team Structure")
        return file_url
    except Exception as error:
        write_exception(error, request)
...
def get_remote_project_csv(payload, request):
    try:
        ...
        file.close()
        file_url = generate_s3_url(file.name, request, "Remote Project")
        return file_url
    except Exception as error:
        write_exception(error, request)
